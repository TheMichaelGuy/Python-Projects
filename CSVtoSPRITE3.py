...
def csv_to_sprite(csv_file : str, costume_asset : dict, sprite_name : str = "Sprite1", make_local : bool = False, split_columns : bool = False):
    with open(csv_file, newline='') as input:
        ...
        # Generate blocks for each row/column 
        for i, h in enumerate(headers): # columns
            for row in rows: # rows
                ...
                block = {
                    ...
                    "shadow": False,
                    "topLevel": prev_block_id is None
                }
                ...
                if prev_block_id:
                    blocks[prev_block_id]["next"] = block_id
...
def export_sprite(sprite : dict, sprite_name : str = "Sprite1"):
    with zipfile.ZipFile(sprite_name + ".sprite3", "w") as output:
        ...
        output.write("bcf454acf82e4504149f7ffe07081dbc.svg", "bcf454acf82e4504149f7ffe07081dbc.svg")

def CSVtoSPRITE3(csv_file : str, in_dir : str, out_dir : str, costume_asset : dict, sprite_name : str = "Sprite1", make_local : bool = False, split_columns : bool = False):
    sprite = csv_to_sprite(csv_file=(in_dir + csv_file), costume_asset=costume_asset, sprite_name=sprite_name, make_local=make_local, split_columns=split_columns)
    export_sprite(sprite=sprite, sprite_name=(out_dir + sprite_name))

""" CODE """

encoded_costume = "bcf454acf82e4504149f7ffe07081dbc"
...
scratch_cat = """<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="95.17898101806641" height="100.04156036376953" viewBox="0.3210171699523926 0.3000000357627869 95.17898101806641 100.04156036376953" version="1.1" xml:space="preserve">
